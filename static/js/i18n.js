...
class I18n {
    ...
    setLocale(locale) {
        if (!this.translations[locale] && locale !== this.defaultLocale) {
            console.warn(
                `[i18n] Locale '${locale}' not found, falling back to '${this.fallbackLocale}'`,
            );
    ...
    updateDOM() {
        ...
        document
            ...
            .forEach((element) => {
                ...
                if (key) {
                    if (this.currentLocale === this.defaultLocale) {
                        const original = this.originalTexts.get(
                            `placeholder:${key}`,
                        );
                        if (original) {
    ...
    updateLanguageSelector() {
        ...
        if (button && dropdown) {
            ...
            dropdown
                .querySelectorAll(".language-dropdown-item")
                .forEach((item) => {
