...
const zhCN = {
    ...
    serverConfig: {
        ...
        chatTemplate: {
            ...
            noteTitle: "ℹ️ 注意：",
            noteContent:
                "vLLM 会自动从每个模型的分词器配置中加载聊天模板和停止令牌。下面的字段仅供参考。现代模型（2023+）都有内置的正确工作的模板。",
            template: "聊天模板（Jinja2）",
            templateHelp: "vLLM 自动使用模型分词器配置中的聊天模板。仅供参考。",
            stopTokens: "停止令牌（逗号分隔）",
            stopTokensHelp:
                "vLLM 通过聊天模板自动处理停止令牌。仅供参考。使用服务器配置中的 'custom_stop_tokens' 来覆盖。",
            referenceOnly: "🔄 仅供参考",
    ...
    chat: {
        ...
        structuredOutputs: {
            ...
            enable: "启用结构化输出",
            cpuWarning:
                "<strong>CPU 模式：</strong>需要 <code>dtype=float32</code>。在 CPU 上使用 bfloat16/float16 会导致错误。",
            outputType: "输出类型：",
            types: {
                choice: "选择",
                regex: "正则表达式",
        ...
        mcp: {
            ...
            infoTip:
                "启用工具调用启动 vLLM。设置最大模型长度为 8192+。使用具有工具调用能力的较大模型（例如 Qwen 2.5 7B+、Llama 3.1 8B+）以获得更好的效果。",
