...
function injectMethods(ui) {
    ...
    ui.pollBenchmarkStatus =
        GuideLLMModule.pollBenchmarkStatus.bind(GuideLLMModule);
    ui.displayBenchmarkResults =
        GuideLLMModule.displayBenchmarkResults.bind(GuideLLMModule);
    ui.displayBenchmarkTable =
        GuideLLMModule.displayBenchmarkTable.bind(GuideLLMModule);
    ...
    ui.updateBenchmarkServerStatus =
        GuideLLMModule.updateBenchmarkServerStatus.bind(GuideLLMModule);

    // Command preview and copy methods
    ui.updateBenchmarkCommandPreview =
        GuideLLMModule.updateBenchmarkCommandPreview.bind(GuideLLMModule);
    ui.copyBenchmarkCommand =
        GuideLLMModule.copyBenchmarkCommand.bind(GuideLLMModule);
    ui.copyGuidellmOutput =
        GuideLLMModule.copyGuidellmOutput.bind(GuideLLMModule);
...
export const GuideLLMModule = {
    ...
    updateStatusIndicator() {
        const statusContainer = document.getElementById("guidellm-status");
        const installWarning = document.getElementById(
            "guidellm-install-warning",
        );
        const metricsContent = document.getElementById(
    ...
    setupBenchmarkListeners() {
        ...
        if (ui.elements.runBenchmarkBtn) {
            ui.elements.runBenchmarkBtn.addEventListener("click", () =>
                ui.runBenchmark(),
            );
        }
        if (ui.elements.stopBenchmarkBtn) {
            ui.elements.stopBenchmarkBtn.addEventListener("click", () =>
                ui.stopBenchmark(),
            );
        }
        ...
        if (ui.elements.benchmarkMethodBuiltin) {
            ui.elements.benchmarkMethodBuiltin.addEventListener("change", () =>
                ui.updateBenchmarkCommandPreview(),
            );
        }
        if (ui.elements.benchmarkMethodGuidellm) {
            ui.elements.benchmarkMethodGuidellm.addEventListener("change", () =>
                ui.updateBenchmarkCommandPreview(),
            );
        }
        ...
        if (ui.elements.copyBenchmarkCommandBtn) {
            ui.elements.copyBenchmarkCommandBtn.addEventListener("click", () =>
                ui.copyBenchmarkCommand(),
            );
        }
        if (ui.elements.copyGuidellmOutputBtn) {
            ui.elements.copyGuidellmOutputBtn.addEventListener("click", () =>
                ui.copyGuidellmOutput(),
            );
        }
        if (ui.elements.copyGuidellmJsonBtn) {
            ui.elements.copyGuidellmJsonBtn.addEventListener("click", () =>
                ui.copyGuidellmJson(),
            );
        }
        ...
        if (ui.elements.toggleRawOutputBtn) {
            ui.elements.toggleRawOutputBtn.addEventListener("click", () =>
                ui.toggleRawOutput(),
            );
        }
        if (ui.elements.toggleJsonOutputBtn) {
            ui.elements.toggleJsonOutputBtn.addEventListener("click", () =>
                ui.toggleJsonOutput(),
            );
        }
    },
    ...
    async runBenchmark() {
        ...
        if (!ui.serverRunning) {
            ui.showNotification(
                "Server must be running to benchmark",
                "warning",
            );
            return;
        }
        ...
        try {
            ...
            ui.benchmarkPollInterval = setInterval(
                () => ui.pollBenchmarkStatus(),
                1000,
            );
        } catch (err) {
    ...
    async pollBenchmarkStatus() {
        ...
        try {
            ...
            if (data.running) {
                ...
                const estimated =
                    (ui.elements.benchmarkRequests.value /
                        ui.elements.benchmarkRate.value) *
                    1000;
                ...
                if (elapsed < estimated) {
                    ...
                    const slowProgress =
                        90 + Math.min(overtime / estimated, 1) * 8;
                ...
                if (data.results) {
                    ...
                    console.error(
                        "[POLL] Benchmark completed but no results:",
                        data,
                    );
    ...
    displayBenchmarkResults(results) {
        ...
        if (isGuideLLM) {
            ...
            ui.elements.metricsGrid.style.display = "none";
            const rawOutputSection = document.getElementById(
                "guidellm-raw-output-section",
            );
            const rawOutputTextarea = document.getElementById(
                "guidellm-raw-output",
            );
            const rawOutputContent = ui.elements.guidellmRawOutputContent;
            const toggleBtn = ui.elements.toggleRawOutputBtn;
            const jsonOutputSection = document.getElementById(
                "guidellm-json-output-section",
            );
            const jsonOutputPre = document.getElementById(
                "guidellm-json-output",
            );
            ...
            if (results.json_output) {
                try {
                    const jsonData =
                        typeof results.json_output === "string"
                            ? JSON.parse(results.json_output)
                            : results.json_output;

                    if (jsonOutputSection && jsonOutputPre) {
                        jsonOutputPre.textContent = JSON.stringify(
                            jsonData,
                            null,
                            2,
                        );
                        ...
                        const jsonOutputContent =
                            ui.elements.guidellmJsonOutputContent;
            ...
            document.getElementById("metric-latency").textContent =
                results.avg_latency !== undefined
                    ? `${results.avg_latency.toFixed(2)} ms`
                    : "-- ms";
            document.getElementById("benchmark-tokens-per-sec").textContent =
                results.tokens_per_second !== undefined
                    ? `${results.tokens_per_second.toFixed(2)} tok/s`
                    : "-- tok/s";
            document.getElementById("metric-p50").textContent =
                results.p50_latency !== undefined
                    ? `${results.p50_latency.toFixed(2)} ms`
                    : "-- ms";
            document.getElementById("metric-p95").textContent =
                results.p95_latency !== undefined
                    ? `${results.p95_latency.toFixed(2)} ms`
                    : "-- ms";
            document.getElementById("metric-p99").textContent =
                results.p99_latency !== undefined
                    ? `${results.p99_latency.toFixed(2)} ms`
                    : "-- ms";
            document.getElementById("benchmark-total-tokens").textContent =
                results.total_tokens !== undefined
                    ? results.total_tokens.toLocaleString()
                    : "--";
            document.getElementById("metric-success-rate").textContent =
                results.success_rate !== undefined
    ...
    displayBenchmarkTable(jsonData) {
        ...
        if (benchmark.metrics) {
            ...
            if (benchmark.metrics.output_tokens_per_second?.successful) {
                const otps =
                    benchmark.metrics.output_tokens_per_second.successful;
            ...
            if (benchmark.metrics.total_tokens_per_second?.successful) {
                const ttps =
                    benchmark.metrics.total_tokens_per_second.successful;
        ...
        if (benchmark.metrics?.request_latency?.successful?.percentiles) {
            ...
            html += '<table class="benchmark-data-table">';
            html +=
                "<thead><tr><th>Percentile</th><th>Latency (s)</th><th>Latency (ms)</th></tr></thead>";
            html += "<tbody>";
