...
try:
    ...
    if not CONTAINER_MODE_AVAILABLE:
        logger.warning(
            "No container runtime (podman/docker) found - container mode will be disabled"
        )
except ImportError:
    try:
...
def validate_local_model_path(model_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        result["info"]["model_name"] = extract_model_name_from_path(
            str(path.resolve()), result["info"]
        )
...
async def debug_connection():
    ...
    if current_config:
        ...
        if current_run_mode == "container" and is_kubernetes and container_manager:
            ...
            namespace = getattr(
                container_manager, "namespace", os.getenv("KUBERNETES_NAMESPACE", "default")
            )
...
async def mcp_save_config(request: MCPServerConfigRequest):
    ...
    if not MCP_AVAILABLE:
        raise HTTPException(
            status_code=400, detail="MCP not installed. Run: pip install vllm-playground[mcp]"
        )
...
async def get_hardware_capabilities():
            ...
            for node in nodes.items:
                ...
                if node.status and node.status.capacity:
                    ...
                    if gpu_capacity and int(gpu_capacity) > 0:
                        ...
                        logger.info(
                            f"GPU detected via Kubernetes API: {node.metadata.name} has {gpu_capacity} GPUs"
                        )
                        break
    ...
    return {"gpu_available": gpu_available, "detection_method": detection_method}
...
async def get_gpu_status():
    ...
    try:
                    ...
                    if is_jetson:
                        ...
                        if temperature == 0:
                            ...
                            if jetson_temp is not None:
                                ...
                                logger.info(
                                    f"Jetson temperature from thermal zone: {temperature}°C"
                                )
        ...
        return {"gpu_available": len(gpu_info) > 0, "gpu_count": len(gpu_info), "gpus": gpu_info}

    except FileNotFoundError:
        logger.info("nvidia-smi not found - no GPU status available")
        return {"gpu_available": False, "gpu_count": 0, "gpus": []}
    except subprocess.TimeoutExpired:
        logger.warning("nvidia-smi timeout")
        return {"gpu_available": False, "gpu_count": 0, "gpus": [], "error": "GPU query timeout"}
    except Exception as e:
        logger.warning(f"Error getting GPU status: {e}")
        return {"gpu_available": False, "gpu_count": 0, "gpus": [], "error": str(e)}
...
async def start_server(config: VLLMConfig):
    ...
    try:
        ...
        if config.run_mode == "container" and (
            not CONTAINER_MODE_AVAILABLE or not container_manager
        ):
            raise HTTPException(
                status_code=400,
        ...
        if not config.use_cpu and config.gpu_device and config.run_mode == "subprocess":
            ...
            await broadcast_log(
                f"[WEBUI] GPU Device Selection - CUDA_VISIBLE_DEVICES={config.gpu_device}"
            )
        ...
        if config.use_cpu:
            ...
            await broadcast_log(
                f"[WEBUI] CPU Optimizations disabled for Apple Silicon compatibility"
            )
        ...
        if config.custom_chat_template:
            ...
            await broadcast_log(
                f"[WEBUI] Using custom chat template from config (overrides model's built-in template)"
            )
        else:
            ...
            await broadcast_log(
                f"[WEBUI] Trusting vLLM to auto-detect chat template from tokenizer_config.json"
            )
            await broadcast_log(
                f"[WEBUI] vLLM will use model's built-in chat template automatically"
            )
        ...
        if config.enable_tool_calling:
            ...
            if tool_parser:
                ...
                await broadcast_log(
                    f"[WEBUI] ⚠️ Tool calling requested but no parser detected for model"
                )
                await broadcast_log(
        ...
        if config.run_mode == "container":
            ...
            if container_info.get("reused", False):
                await broadcast_log(
                    f"[WEBUI] ⚡ Restarted existing container: {container_id[:12]} (fast!)"
                )
            else:
            ...
            if config.use_cpu:
                ...
                await broadcast_log(
                    f"[WEBUI] Mode: GPU (Memory: {int(config.gpu_memory_utilization * 100)}%)"
                )
            ...
            if readiness.get("ready"):
                await broadcast_log(
                    f"[WEBUI] ✅ vLLM is ready! (took {readiness['elapsed_time']}s)"
                )
                return {
                    "status": "ready",
                ...
                if error_msg == "timeout":
                    await broadcast_log(
                        f"[WEBUI] ⚠️ Warning: vLLM did not become ready within {elapsed}s"
                    )
                    await broadcast_log(
                        f"[WEBUI] Container is running but may still be initializing..."
                    )
                    await broadcast_log(
                        f"[WEBUI] Check the logs above for model download/loading progress"
                    )
                    await broadcast_log(
                    ...
                    await broadcast_log(
                        f"[WEBUI] ❌ Error: Container stopped unexpectedly after {elapsed}s"
                    )
                    ...
                    await broadcast_log(
                        f"[WEBUI] ⚠️ Warning: Could not verify readiness: {error_msg}"
                    )
                    await broadcast_log(
                        f"[WEBUI] Container may still be working - check logs for details"
                    )
    ...
    try:
        ...
        if request.tools:
            ...
            for i, msg in enumerate(messages_dict):
                ...
                messages_dict.insert(
                    ...
                    {
                        "role": "system",
                        "content": f"You are a helpful assistant.{tool_format_hint}",
                    },
                )
        ...
        if request.tool_choice is not None:
            ...
            if not request.tools or len(request.tools) == 0:
                logger.warning(
                    f"⚠️ tool_choice '{request.tool_choice}' provided but no tools defined - ignoring"
                )
            else:
                if isinstance(request.tool_choice, str):
                    ...
                    logger.info(
                        f"🔧 Tool choice: {payload.get('tool_choice', request.tool_choice)}"
                    )
                else:
                    ...
                    logger.info(
                        f"🔧 Tool choice: specific function - {request.tool_choice.function}"
                    )
        ...
        if request.response_format:
            ...
            if (
                ...
                logger.info(
                    f"📋 JSON Schema structured output enabled: {request.response_format.json_schema.name}"
                )
        ...
        if current_config.custom_stop_tokens:
            ...
            logger.info(
                f"Using custom stop tokens from server config: {current_config.custom_stop_tokens}"
            )
            ...
            logger.warning(
                f"Using stop tokens from request (not recommended): {request.stop_tokens}"
            )
        else:
            # Let vLLM handle stop tokens automatically from model's tokenizer (RECOMMENDED)
            logger.info(
                f"✓ Letting vLLM handle stop tokens automatically (recommended for /v1/chat/completions)"
            )
                                                        ...
                                                        if (
                                                            ...
                                                            if finish_reason:
                                                                ...
                                                                if (
                                                                    ...
                                                                    logger.warning(
                                                                        f"⚠️ Full chunk data: {data}"
                                                                    )
                                                except Exception as parse_err:
                                                    logger.debug(
                                                        f"Failed to parse SSE data: {parse_err}"
                                                    )
                ...
                async with session.post(url, json=payload) as response:
                    if response.status != 200:
                        ...
                        error_detail = (
                            text.strip()
                            if text.strip()
                            else f"vLLM server returned HTTP {response.status}"
                        )
        ...
        error_msg = (
            f"Connection error to vLLM server: {type(e).__name__}: {str(e) or 'Unknown error'}"
        )
    ...
    for tool in request.tools:
        ...
        if tool.type != "function":
            tool_result["errors"].append(
                f"Invalid tool type: '{tool.type}'. Only 'function' is supported."
            )
        ...
        if tool.function.parameters:
            ...
            if "type" not in params:
                tool_result["warnings"].append(
                    "Parameters schema missing 'type' field. Should be 'object'."
                )
            elif params["type"] != "object":
                tool_result["warnings"].append(
                    f"Parameters type is '{params['type']}'. Usually should be 'object'."
                )
...
async def get_recipes_by_category(category_id: str):
    ...
    try:
        ...
        return JSONResponse(
            status_code=404, content={"error": f"Category '{category_id}' not found"}
        )
    except Exception as e:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if recipes_file.exists():
                with open(recipes_file, "r") as f:
                    ...
                    catalog_info = {
                        ...
                        "total_recipes": sum(
                            len(cat.get("recipes", [])) for cat in catalog.get("categories", [])
                        ),
                    }
            ...
            return {
                ...
                else result.stdout,  # Limit output size
            }
        else:
        ...
        return JSONResponse(
            status_code=500,
            content={"success": False, "error": str(type(e).__name__), "message": str(e)},
        )
...
async def save_recipe(request: dict):
    ...
    try:
        ...
        if not recipes_file.exists():
            return JSONResponse(
                status_code=404, content={"success": False, "error": "Recipes catalog not found"}
            )
        ...
        if not is_new and original_category_id and original_category_id != category_id:
            for cat in catalog.get("categories", []):
                if cat["id"] == original_category_id:
                    cat["recipes"] = [
                        r for r in cat.get("recipes", []) if r["id"] != original_recipe_id
                    ]
...
async def delete_recipe(request: dict):
    ...
    try:
        ...
        if not recipe_deleted:
            return JSONResponse(
                status_code=404, content={"success": False, "error": "Recipe not found"}
            )
...
async def browse_directories(request: dict):
    ...
    try:
        ...
        try:
            ...
            for item in sorted(path.iterdir()):
                if item.is_dir() and not item.name.startswith("."):
                    ...
                    directories.append(
                        {"name": item.name + (" 🤖" if is_model_dir else ""), "path": str(item)}
                    )
        ...
        return JSONResponse(
            status_code=500, content={"error": f"Failed to browse directories: {str(e)}"}
        )
...
async def validate_local_model(
    request: LocalModelValidationRequest,
) -> LocalModelValidationResponse:
    ...
    try:
        ...
        if not model_path.exists():
            return LocalModelValidationResponse(
                valid=False, message=f"Path does not exist: {request.path}"
            )

        # Check if it's a directory
        if not model_path.is_dir():
            return LocalModelValidationResponse(
                valid=False, message=f"Path must be a directory, not a file"
            )
...
async def get_vllm_metrics():
    ...
    try:
        ...
        if current_run_mode == "container" and is_kubernetes:
            ...
            metrics_url = (
                f"http://{service_name}.{namespace}.svc.cluster.local:{current_config.port}/metrics"
            )
        else:
            # Subprocess mode or local container mode - connect to localhost
            # Use localhost for container mode since 0.0.0.0 is a bind address, not a valid destination
                    ...
                    if response.status == 200:
                        ...
                        for line in text.split("\n"):
                            if "vllm:gpu_cache_usage_perc" in line and not line.startswith("#"):
                                ...
                                "vllm:avg_prompt_throughput_toks_per_s" in line
                                ...
                                "vllm:avg_generation_throughput_toks_per_s" in line
                                and not line.startswith("#")
                            ):
                                try:
                                    value = float(line.split()[-1])
...
async def run_benchmark(config: BenchmarkConfig, server_config: VLLMConfig):
    ...
    try:
                    ...
                    async with session.post(
                        ...
                        if response.status == 200:
                            ...
                            if i < 3:
                                ...
                                logger.info(
                                    f"[BENCHMARK DEBUG] Request {i + 1} completion_tokens: {completion_tokens}"
                                )
        ...
        if results:
            ...
            logger.info(
                f"[BENCHMARK DEBUG] Total prompt tokens: {len(results) * config.prompt_tokens}"
            )
    ...
    try:
        ...
        try:
            ...
            if check_result.returncode != 0:
                ...
                await broadcast_log(
                    f"[GUIDELLM] WARNING: guidellm CLI not accessible from {python_exec}"
                )
            ...
            await broadcast_log(
                f"[GUIDELLM] WARNING: CLI check timed out (30s), will attempt to run benchmark anyway"
            )
            await broadcast_log(
        ...
        try:
            ...
            if os.path.exists(result_file.name):
                ...
                await broadcast_log(
                    f"[GUIDELLM] 📄 JSON file found: {result_file.name} (size: {file_size} bytes)"
                )
                ...
                await broadcast_log(
                    f"[GUIDELLM] ⚠️ JSON output file not found at {result_file.name}"
                )
                await broadcast_log(
                    f"[GUIDELLM] Checking if guidellm created a file in current directory..."
                )
                ...
                if json_files:
                    await broadcast_log(
                        f"[GUIDELLM] Found JSON files in current directory: {json_files}"
                    )
        except Exception as e:
        ...
        try:
            ...
            await broadcast_log(
                f"[GUIDELLM] 📊 Throughput: {benchmark_results.throughput:.2f} req/s"
            )
            await broadcast_log(
                f"[GUIDELLM] ⚡ Token Throughput: {benchmark_results.tokens_per_second:.2f} tok/s"
            )
            await broadcast_log(
                f"[GUIDELLM] ⏱️  Avg Latency: {benchmark_results.avg_latency:.2f} ms"
            )
            await broadcast_log(
                f"[GUIDELLM] 📈 P99 Latency: {benchmark_results.p99_latency:.2f} ms"
            )
