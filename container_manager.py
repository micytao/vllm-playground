...
class VLLMContainerManager:
    ...
    DEFAULT_IMAGE_GPU = (
        "docker.io/vllm/vllm-openai:v0.11.0"  # Official vLLM GPU image (linux/amd64)
    )
    DEFAULT_IMAGE_CPU_MACOS = (
        "quay.io/rh_ee_micyang/vllm-mac:v0.11.0"  # CPU image for macOS (linux/arm64)
    )
    ...
    async def _run_podman_cmd_async(
        self, *args, capture_output=True, check=True
    ) -> subprocess.CompletedProcess:
    ...
    def build_container_config(self, vllm_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        env.extend(
            ["-e", f"VLLM_MODEL={vllm_config.get('model_source', vllm_config.get('model'))}"]
        )
        ...
        if vllm_config.get("use_cpu", False):
            ...
            env.extend(
                [
                    "-e",
                    f"VLLM_CPU_OMP_THREADS_BIND={vllm_config.get('cpu_omp_threads_bind', 'auto')}",
                ]
            )
        ...
        if not vllm_config.get("use_cpu", False):
            env.extend(
                ["-e", f"VLLM_TENSOR_PARALLEL_SIZE={vllm_config.get('tensor_parallel_size', 1)}"]
            )
            env.extend(
                [
                    "-e",
                    f"VLLM_GPU_MEMORY_UTILIZATION={vllm_config.get('gpu_memory_utilization', 0.9)}",
                ]
            )
        ...
        if not vllm_config.get("use_cpu", False):
            ...
            vllm_args.extend(
                ["--gpu-memory-utilization", str(vllm_config.get("gpu_memory_utilization", 0.9))]
            )
        ...
        if enable_tool_calling:
            ...
            if tool_parser:
                ...
                logger.warning(
                    f"Tool calling enabled but no parser found for model: {model_source}"
                )
        else:
    ...
    async def _should_recreate_container(
        self, vllm_config: Dict[str, Any], expected_image: str
    ) -> bool:
        """
    ...
    async def _pull_image_with_progress(self, image: str) -> bool:
        ...
        logger.info(
            f"Pulling container image: {image} (this may take several minutes for large images...)"
        )
            ...
            if not should_recreate:
                ...
                if current_status == "running":
                    ...
                    id_result = await self._run_podman_cmd_async(
                        "inspect", self.CONTAINER_NAME, "--format", "{{.Id}}"
                    )
