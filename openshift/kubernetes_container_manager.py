...
class VLLMKubernetesManager:
    ...
    def build_pod_spec(
        self, vllm_config: Dict[str, Any], image: Optional[str] = None
    ) -> client.V1Pod:
        """
        ...
        env_vars.append(
            client.V1EnvVar(
                name="VLLM_MODEL", value=vllm_config.get("model_source", vllm_config.get("model"))
            )
        )
        ...
        if vllm_config.get("use_cpu", False) and vllm_config.get("dtype", "auto") == "auto":
            ...
            env_vars.append(
                client.V1EnvVar(name="VLLM_DTYPE", value=vllm_config.get("dtype", "auto"))
            )
        ...
        if vllm_config.get("hf_token"):
            ...
            env_vars.append(
                client.V1EnvVar(name="HUGGING_FACE_HUB_TOKEN", value=vllm_config["hf_token"])
            )
        ...
        if vllm_config.get("use_cpu", False):
            env_vars.append(
                client.V1EnvVar(
                    ...
                    value=str(vllm_config.get("cpu_kvcache_space", 4)),
                )
            )
            env_vars.append(
                client.V1EnvVar(
                    name="VLLM_CPU_OMP_THREADS_BIND",
                    value=vllm_config.get("cpu_omp_threads_bind", "auto"),
                )
            )
        ...
        if not vllm_config.get("use_cpu", False):
            ...
            env_vars.append(
                client.V1EnvVar(
                    ...
                    value=str(vllm_config.get("tensor_parallel_size", 1)),
                )
            )
            env_vars.append(
                client.V1EnvVar(
                    name="VLLM_GPU_MEMORY_UTILIZATION",
                    value=str(vllm_config.get("gpu_memory_utilization", 0.9)),
                )
            )
            env_vars.append(
                client.V1EnvVar(
                    name="VLLM_LOAD_FORMAT", value=vllm_config.get("load_format", "auto")
                )
            )
        ...
        if vllm_config.get("custom_chat_template"):
            ...
            logger.info(
                "Using custom chat template from config (overrides model's built-in template)"
            )
        else:
        ...
        metadata = client.V1ObjectMeta(
            name=self.POD_NAME, labels={"app": "vllm", "managed-by": "vllm-playground"}
        )
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        f"{service_url}/health", timeout=aiohttp.ClientTimeout(total=3)
                    ) as response:
            ...
            if elapsed < timeout:
                ...
                if int(elapsed) % 15 == 0:  # Log every 15 seconds
                    logger.info(
                        f"Still waiting for vLLM... ({int(elapsed)}s elapsed, last error: {last_error})"
                    )
            ...
            while True:
                try:
                    ...
                    if not line:
                        # No more data
                        logger.info(
                            f"Log stream ended for pod {self.POD_NAME} (read {line_count} lines)"
                        )
                        break
